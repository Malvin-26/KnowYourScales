...
const insertScale = db.prepare(`
  INSERT OR REPLACE INTO scales (id, root, type, notes, formula, relative_key)
  VALUES (?, ?, ?, ?, ?, ?)
`);
...
const insertProg = db.prepare(`
  INSERT OR REPLACE INTO chord_progressions (id, name, genre, key_example, numerals, chords, description)
  VALUES (?, ?, ?, ?, ?, ?, ?)
`);

console.log('Seeding chord progressions...');
for (const p of progressions) {
  insertProg.run(p.id, p.name, p.genre, p.key_example, p.numerals, p.chords, p.description);
}
...
const insertSong = db.prepare(`
  INSERT OR REPLACE INTO songs (id, title, artist, scale_key, difficulty, tempo, progression, chords, category)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
for (const s of songs) {
  insertSong.run(
    s.id, s.title, s.artist, s.scale_key, s.difficulty,
    s.tempo, s.progression, s.chords, s.category
  );
}
...
const insertAch = db.prepare(`
  INSERT OR REPLACE INTO achievements (id, name, description, icon, xp_reward, category)
  VALUES (?, ?, ?, ?, ?, ?)
`);

console.log('Seeding achievements...');
for (const a of achievements) {
  insertAch.run(a.id, a.name, a.description, a.icon, a.xp_reward, a.category);
}
